    ...
    async def _read_from_socket(self, command: ProtocolCommand) -> ProtocolResponse:
        ...
        if filename is not None:
            ...
            return ProtocolResponse(
                bytes.fromhex("aa55f700010203040506070809"), command
            )
...
class GW10K_ET_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        ...
        self.mock_response(
            self._READ_DEVICE_VERSION_INFO, "GW10K-ET_device_info_fw617.hex"
        )
    ...
    def test_GW10K_ET_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2021-08-22 11:11:12", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        ...
        self.assertSensor(
            "pv1_mode_label", "PV panels connected, producing power", "", data
        )
        self.assertSensor("pv2_mode", 2, "", data)
        self.assertSensor(
            "pv2_mode_label", "PV panels connected, producing power", "", data
        )
    ...
    def test_set_operation_mode_ECO_CHARGE(self):
        ...
        self.assertEqual(
            "f710b99b0004080000173bffd8ff7f1343", self._list_of_requests[-9].hex()
        )
        self.loop.run_until_complete(
            self.set_operation_mode(
                OperationMode.ECO_CHARGE, eco_mode_power=40, eco_mode_soc=80
            )
        )
        self.assertEqual(
            "f710b99b0004080000173bffd8ff7f1343", self._list_of_requests[-9].hex()
    ...
    def test_set_operation_mode_DISCHARGE(self):
        self.loop.run_until_complete(self.read_device_info())
        self.loop.run_until_complete(
            self.set_operation_mode(OperationMode.ECO_DISCHARGE, eco_mode_power=50)
        )
        self.assertEqual(
...
class GW10K_ET_fw819_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        EtMock.__init__(self, methodName)
        self.mock_response(
            self._READ_DEVICE_VERSION_INFO, "GW10K-ET_device_info_fw819.hex"
        )
    ...
    def test_set_operation_mode_ECO_CHARGE(self):
        ...
        self.assertEqual(
            "f710b9bb00060c0000173bff7fffd80050000002cc",
            self._list_of_requests[-9].hex(),
        )
        self.loop.run_until_complete(
            self.set_operation_mode(OperationMode.ECO_CHARGE, eco_mode_power=40)
        )
        self.assertEqual(
            "f710b9bb00060c0000173bff7fffd8006400004302",
            self._list_of_requests[-9].hex(),
        )
    ...
    def test_set_operation_mode_ECO_DISCHARGE(self):
        ...
        self.assertEqual(
            "f710b9bb00060c0000173bff7f0032006400004eda",
            self._list_of_requests[-9].hex(),
        )
...
class GW10K_ET_fw1023_Test(EtMock):
    ...
    def test_GW10K_ET_runtime_data_fw1023(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2024-05-11 00:03:34", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        ...
        self.assertSensor(
            "diagnose_result_label", "Discharge Driver On, PF value set", "", data
        )
...
class GEH10_1U_10_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        ...
        self.mock_response(
            self._READ_DEVICE_VERSION_INFO, "GEH10-1U-10_device_info.hex"
        )
    ...
    def test_GEH10_1U_10_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2023-01-26 11:34:07", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        ...
        self.assertSensor(
            "pv4_mode_label", "PV panels connected, producing power", "", data
        )
        self.assertSensor("pv3_mode", 2, "", data)
        self.assertSensor(
            "pv3_mode_label", "PV panels connected, producing power", "", data
        )
        self.assertSensor("pv2_mode", 2, "", data)
        self.assertSensor(
            "pv2_mode_label", "PV panels connected, producing power", "", data
...
class GW6000_ES_20_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        EtMock.__init__(self, methodName)
        self.mock_response(
            self._READ_DEVICE_VERSION_INFO, "GW6000-ES-20_device_info.hex"
        )
...
class GW20K_ET_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        EtMock.__init__(self, methodName)
        self.mock_response(self._READ_DEVICE_VERSION_INFO, "GW20K-ET_device_info.hex")
        self.mock_response(self._READ_RUNNING_DATA, "GW20K-ET_running_data.hex")
        self.mock_response(self._READ_METER_DATA_EXTENDED2, "GW20K-ET_meter_data.hex")
        self.mock_response(self._READ_BATTERY_INFO, "GW20K-ET_battery_info.hex")
        self.mock_response(self._READ_MPPT_DATA, "GW20K-ET_mppt_data.hex")

    def test_GW20K_ET_device_info(self):
        self.loop.run_until_complete(self.read_device_info())
        self.assertEqual("", self.model_name)
        self.assertEqual("9020KETT232W0000", self.serial_number)
        self.assertEqual(20000, self.rated_power)
        self.assertEqual(0, self.modbus_version)
        self.assertEqual(1, self.ac_output_type)
        self.assertEqual(6, self.dsp1_version)
        self.assertEqual(6, self.dsp2_version)
        self.assertEqual(8045, self.dsp_svn_version)
        self.assertEqual(8, self.arm_version)
        self.assertEqual(407, self.arm_svn_version)
        self.assertEqual("04062-08-S00", self.firmware)
        self.assertEqual("02020-10-S01", self.arm_firmware)

    def test_GW20K_ET_runtime_data(self):
        # Reset sensors
        self.loop.run_until_complete(self.read_device_info())
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2026-03-29 17:26:45", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        self.assertSensor('vpv1', 644.3, 'V', data)
        self.assertSensor('ipv1', 1.7, 'A', data)
        self.assertSensor('ppv1', 1098, 'W', data)
        self.assertSensor('vpv2', 644.3, 'V', data)
        self.assertSensor('ipv2', 0.0, 'A', data)
        self.assertSensor('ppv2', 997, 'W', data)
        self.assertSensor('vpv3', 569.3, 'V', data)
        self.assertSensor('ipv3', 1.8, 'A', data)
        self.assertSensor('ppv3', 0, 'W', data)
        self.assertSensor('vpv4', 569.3, 'V', data)
        self.assertSensor('ipv4', 0.0, 'A', data)
        self.assertSensor('ppv4', 0, 'W', data)
        self.assertSensor('ppv', 2095, 'W', data)
        ...
        self.assertSensor('vgrid', 233.8, 'V', data)
        self.assertSensor('igrid', 2.9, 'A', data)
        self.assertSensor('fgrid', 49.99, 'Hz', data)
        self.assertSensor('pgrid', 651, 'W', data)
        self.assertSensor('vgrid2', 233.7, 'V', data)
        self.assertSensor('igrid2', 3.0, 'A', data)
        self.assertSensor('fgrid2', 50.0, 'Hz', data)
        self.assertSensor('pgrid2', 652, 'W', data)
        self.assertSensor('vgrid3', 234.7, 'V', data)
        self.assertSensor('igrid3', 3.0, 'A', data)
        self.assertSensor('fgrid3', 49.99, 'Hz', data)
        self.assertSensor('pgrid3', 663, 'W', data)
        self.assertSensor('grid_mode', 1, '', data)
        self.assertSensor('grid_mode_label', "Connected to grid", '', data)
        self.assertSensor('total_inverter_power', 1966, 'W', data)
        self.assertSensor('active_power', 1556, 'W', data)
        self.assertSensor('grid_in_out', 1, '', data)
        self.assertSensor('grid_in_out_label', "Exporting", '', data)
        self.assertSensor('reactive_power', 827, 'var', data)
        self.assertSensor('apparent_power', 2083, 'VA', data)
        self.assertSensor('backup_v1', 233.2, 'V', data)
        self.assertSensor('backup_i1', 1.7, 'A', data)
        self.assertSensor('backup_f1', 49.99, 'Hz', data)
        self.assertSensor('load_mode1', 0, '', data)
        self.assertSensor('backup_p1', 235, 'W', data)
        self.assertSensor('backup_v2', 233.7, 'V', data)
        self.assertSensor('backup_i2', 1.2, 'A', data)
        self.assertSensor('backup_f2', 50.0, 'Hz', data)
        self.assertSensor('load_mode2', 0, '', data)
        self.assertSensor('backup_p2', 140, 'W', data)
        self.assertSensor('backup_v3', 234.1, 'V', data)
        self.assertSensor('backup_i3', 0.4, 'A', data)
        self.assertSensor('backup_f3', 49.99, 'Hz', data)
        self.assertSensor('load_mode3', 0, '', data)
        self.assertSensor('backup_p3', 91, 'W', data)
        self.assertSensor('load_p1', 221, 'W', data)
        self.assertSensor('load_p2', 125, 'W', data)
        self.assertSensor('load_p3', 65, 'W', data)
        self.assertSensor('backup_ptotal', 484, 'W', data)
        self.assertSensor('load_ptotal', 0, 'W', data)
        self.assertSensor('ups_load', 4, '%', data)
        self.assertSensor('temperature_air', 43.9, 'C', data)
        self.assertSensor('temperature_module', 0.0, 'C', data)
        self.assertSensor('temperature', 42.2, 'C', data)
        self.assertSensor('function_bit', 1, '', data)
        self.assertSensor('bus_voltage', 762.5, 'V', data)
        self.assertSensor('nbus_voltage', 381.3, 'V', data)
        self.assertSensor('vbattery1', 381.6, 'V', data)
        self.assertSensor('ibattery1', -0.4, 'A', data)
        self.assertSensor('pbattery1', -153, 'W', data)
        self.assertSensor('battery_mode', 3, '', data)
        self.assertSensor('battery_mode_label', "Charge", '', data)
        self.assertSensor('warning_code', 0, '', data)
        self.assertSensor('safety_country', 2, '', data)
        self.assertSensor('safety_country_label', "DE LV with PV", '', data)
        self.assertSensor('work_mode', 1, '', data)
        self.assertSensor('work_mode_label', "Normal (On-Grid)", '', data)
        self.assertSensor('operation_mode', 0, '', data)
        self.assertSensor('error_codes', 0, '', data)
        self.assertSensor('errors', "", '', data)
        self.assertSensor('e_total', 50.1, 'kWh', data)
        self.assertSensor('e_day', 49.9, 'kWh', data)
        self.assertSensor('e_total_exp', 331.6, 'kWh', data)
        self.assertSensor('h_total', 493, 'h', data)
        self.assertSensor('e_day_exp', 40.8, 'kWh', data)
        self.assertSensor('e_total_imp', 392.0, 'kWh', data)
        self.assertSensor('e_day_imp', 0.0, 'kWh', data)
        self.assertSensor('e_load_total', 285.6, 'kWh', data)
        self.assertSensor('e_load_day', 0.2, 'kWh', data)
        self.assertSensor('e_bat_charge_total', 381.7, 'kWh', data)
        self.assertSensor('e_bat_charge_day', 10.4, 'kWh', data)
        self.assertSensor('e_bat_discharge_total', 303.6, 'kWh', data)
        self.assertSensor('e_bat_discharge_day', 2.8, 'kWh', data)
        self.assertSensor('diagnose_result', 318767360, '', data)
        self.assertSensor('diagnose_result_label', "APP: Discharge current too low, Export power limit set, PF value set, SOC protect off", '', data)
        self.assertSensor('house_consumption', 386, 'W', data)
        self.assertSensor('commode', 7, '', data)
        self.assertSensor('rssi', 0, '', data)
        self.assertSensor('manufacture_code', 10, '', data)
        self.assertSensor('meter_test_status', 273, '', data)
        self.assertSensor('meter_comm_status', 1, '', data)
        self.assertSensor('active_power1', 430, 'W', data)
        self.assertSensor('active_power2', 527, 'W', data)
        self.assertSensor('active_power3', 598, 'W', data)
        self.assertSensor('active_power_total', 1556, 'W', data)
        self.assertSensor('reactive_power_total', 82, 'var', data)
        self.assertSensor('meter_power_factor1', 0.084, '', data)
        self.assertSensor('meter_power_factor2', 0.09, '', data)
        self.assertSensor('meter_power_factor3', 0.091, '', data)
        self.assertSensor('meter_power_factor', 0.089, '', data)
        self.assertSensor('meter_freq', 50.0, 'Hz', data)
        self.assertSensor('meter_active_power1', 430, 'W', data)
        self.assertSensor('meter_active_power2', 527, 'W', data)
        self.assertSensor('meter_active_power3', 598, 'W', data)
        self.assertSensor('meter_active_power_total', 1556, 'W', data)
        self.assertSensor('meter_reactive_power1', 102, 'var', data)
        self.assertSensor('meter_reactive_power2', 19, 'var', data)
        self.assertSensor('meter_reactive_power3', -39, 'var', data)
        self.assertSensor('meter_reactive_power_total', 82, 'var', data)
        self.assertSensor('meter_apparent_power1', 506, 'VA', data)
        self.assertSensor('meter_apparent_power2', 581, 'VA', data)
        self.assertSensor('meter_apparent_power3', 652, 'VA', data)
        self.assertSensor('meter_apparent_power_total', 1740, 'VA', data)
        ...
        self.assertSensor('meter_voltage1', 233.8, 'V', data)
        self.assertSensor('meter_voltage2', 233.9, 'V', data)
        self.assertSensor('meter_voltage3', 233.7, 'V', data)
        self.assertSensor('meter_current1', 2.1, 'A', data)
        self.assertSensor('meter_current2', 2.4, 'A', data)
        self.assertSensor('meter_current3', 2.8, 'A', data)
        self.assertSensor('meter_e_total_exp1', 10.72, 'kWh', data)
        self.assertSensor('meter_e_total_exp2', 12.61, 'kWh', data)
        self.assertSensor('meter_e_total_exp3', 13.9, 'kWh', data)
        self.assertSensor('meter_e_total_exp', 35.55, 'kWh', data)
        self.assertSensor('meter_e_total_imp1', 1.33, 'kWh', data)
        self.assertSensor('meter_e_total_imp2', 0.49, 'kWh', data)
        self.assertSensor('meter_e_total_imp3', 0.02, 'kWh', data)
        self.assertSensor('meter_e_total_imp', 0.15, 'kWh', data)
        self.assertSensor('battery_bms', 1, '', data)
        self.assertSensor('battery_index', 498, '', data)
        self.assertSensor('battery_status', 1, '', data)
        self.assertSensor('battery_temperature', 14.1, 'C', data)
        self.assertSensor('battery_charge_limit', 0, 'A', data)
        self.assertSensor('battery_discharge_limit', 30, 'A', data)
        self.assertSensor('battery_error_l', 0, '', data)
        self.assertSensor('battery_soc', 100, '%', data)
        self.assertSensor('battery_soh', 100, '%', data)
        self.assertSensor('battery_modules', 7, '', data)
        self.assertSensor('battery_warning_l', 0, '', data)
        self.assertSensor('battery_protocol', 286, '', data)
        self.assertSensor('battery_error_h', 0, '', data)
        self.assertSensor('battery_error', "", '', data)
        self.assertSensor('battery_warning_h', 0, '', data)
        self.assertSensor('battery_warning', "", '', data)
        self.assertSensor('battery_sw_version', 2502, '', data)
        self.assertSensor('battery_hw_version', 2752, '', data)
        self.assertSensor('battery_max_cell_temp_id', 0, '', data)
        self.assertSensor('battery_min_cell_temp_id', 0, '', data)
        self.assertSensor('battery_max_cell_voltage_id', 0, '', data)
        self.assertSensor('battery_min_cell_voltage_id', 0, '', data)
        self.assertSensor('battery_max_cell_temp', 14.1, 'C', data)
        self.assertSensor('battery_min_cell_temp', 9.0, 'C', data)
        self.assertSensor('battery_max_cell_voltage', 4.0, 'V', data)
        self.assertSensor('battery_min_cell_voltage', 3.9610000000000003, 'V', data)
        self.assertSensor('ppv_total', 2095, 'W', data)
        ...
        self.assertSensor('pmppt1', 1098, 'W', data)
        self.assertSensor('pmppt2', 997, 'W', data)
        ...
        self.assertSensor('imppt1', 1.7, 'A', data)
        self.assertSensor('imppt2', 1.8, 'A', data)
        self.assertSensor('imppt3', 0.0, 'A', data)
...
class GW25K_ET_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        ...
        self.mock_response(
            self._READ_BATTERY2_INFO_EXTENDED, "GW25K-ET_battery2_info_extended.hex"
        )
    ...
    def test_GW25K_ET_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2023-12-03 14:07:07", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        ...
        self.assertSensor("apparent_power2", 0, "VA", data)
        self.assertSensor("apparent_power3", 0, "VA", data)
        self.assertSensor("vbattery2", 417.8, "V", data)
        self.assertSensor("ibattery2", -3.0, "A", data)
...
class GW29K9_ET_Test(EtMock):
    ...
    def __init__(self, methodName="runTest"):
        ...
        self.mock_response(
            self._READ_BATTERY2_INFO_EXTENDED, "GW29K9-ET_battery2_info_extended.hex"
        )
    ...
    def test_GW29K9_ET_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2024-01-17 14:49:14", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
