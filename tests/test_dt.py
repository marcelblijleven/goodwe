    ...
    async def _read_from_socket(self, command: ProtocolCommand) -> ProtocolResponse:
        ...
        if filename is not None:
            ...
            return ProtocolResponse(
                bytes.fromhex("aa557f00010203040506070809"), command
            )
...
class GW6000_DT_Test(DtMock):
    ...
    def test_GW6000_DT_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2021-08-31 12:03:02", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW8K_DT_Test(DtMock):
    ...
    def test_GW8K_DT_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2021-08-24 16:43:27", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW5000D_NS_Test(DtMock):
    ...
    def test_GW5000D_NS_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2021-09-06 06:56:01", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW5000_MS_Test(DtMock):
    ...
    def test_GW5000_MS_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2021-10-15 09:03:12", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW10K_MS_30_Test(DtMock):
    ...
    def __init__(self, methodName="runTest"):
        DtMock.__init__(self, methodName)
        self.mock_response(
            self._READ_DEVICE_VERSION_INFO, "GW10K-MS-30_device_info.hex"
        )
    ...
    def test_GW10K_MS_30_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2024-01-09 22:08:20", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW10K_MS_TCP_Test(DtMock):
    ...
    def test_GW10K_MS_TCP_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2024-06-02 09:07:17", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW20KAU_DT_Test(DtMock):
    ...
    def test_GW20KAU_DT_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2022-10-21 19:23:42", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        ...
        self.assertSensor(
            "derating_mode_label", "Reactive power derating(PF/QU/FixQ)", "", data
        )
...
class GW17K_DT_Test(DtMock):
    ...
    def test_GW20KAU_DT_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2024-05-20 10:35:55", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
...
class GW50KS_MT_Test(DtMock):

    def __init__(self, methodName="runTest"):
        DtMock.__init__(self, methodName, 8899)
        self.mock_response(self._READ_DEVICE_VERSION_INFO, "GW50KS-MT_device_info.hex")
        self.mock_response(self._READ_RUNNING_DATA, "GW50KS-MT_running_data.hex")
        self.mock_response(self._READ_METER_DATA, ILLEGAL_DATA_ADDRESS)

    def test_GW50KS_MT_device_info(self):
        self.loop.run_until_complete(self.read_device_info())
        self.assertEqual("GW50KS-MT", self.model_name)
        self.assertEqual("5050KMTO233W0000", self.serial_number)
        self.assertEqual(4, self.dsp1_version)
        self.assertEqual(4, self.dsp2_version)
        self.assertEqual(168, self.dsp_svn_version)
        self.assertEqual(16, self.arm_version)
        self.assertEqual(345, self.arm_svn_version)
        self.assertEqual("4.4.10", self.firmware)
    ...
    def test_GW50KS_MT_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2026-03-29 12:21:58", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )

        self.assertSensor('vpv1', 708.0, 'V', data)
        self.assertSensor('ipv1', 16.4, 'A', data)
        self.assertSensor('ppv1', 11611, 'W', data)
        self.assertSensor('vpv2', 723.3, 'V', data)
        self.assertSensor('ipv2', 14.5, 'A', data)
        self.assertSensor('ppv2', 10488, 'W', data)
        self.assertSensor('vpv3', 740.2, 'V', data)
        self.assertSensor('ipv3', 17.9, 'A', data)
        self.assertSensor('ppv3', 13250, 'W', data)
        self.assertSensor('vpv4', 746.3, 'V', data)
        self.assertSensor('ipv4', 17.5, 'A', data)
        self.assertSensor('ppv4', 13060, 'W', data)
        self.assertSensor('ppv', 48409, 'W', data)
        self.assertSensor('vline1', 407.0, 'V', data)
        self.assertSensor('vline2', 405.2, 'V', data)
        self.assertSensor('vline3', 405.6, 'V', data)
        self.assertSensor('vgrid1', 235.0, 'V', data)
        self.assertSensor('vgrid2', 234.9, 'V', data)
        self.assertSensor('vgrid3', 232.9, 'V', data)
        self.assertSensor('igrid1', 80.6, 'A', data)
        self.assertSensor('igrid2', 80.7, 'A', data)
        self.assertSensor('igrid3', 80.6, 'A', data)
        self.assertSensor('fgrid1', 50.01, 'Hz', data)
        self.assertSensor('fgrid2', 50.02, 'Hz', data)
        self.assertSensor('fgrid3', 50.01, 'Hz', data)
        self.assertSensor('pgrid1', 18941, 'W', data)
        self.assertSensor('pgrid2', 18956, 'W', data)
        self.assertSensor('pgrid3', 18772, 'W', data)
        self.assertSensor('total_inverter_power', 47967, 'W', data)
        self.assertSensor('work_mode', 1, '', data)
        self.assertSensor('work_mode_label', "Normal", '', data)
        self.assertSensor('error_codes', 0, '', data)
        self.assertSensor('warning_code', 0, '', data)
        self.assertSensor('apparent_power', 56464, 'VA', data)
        self.assertSensor('reactive_power', -29840, 'var', data)
        self.assertSensor('total_input_power', 0, 'W', data)
        self.assertSensor('power_factor', 0.849, '', data)
        self.assertSensor('temperature', 34.2, 'C', data)
        self.assertSensor('temperature_heatsink', 37.7, 'C', data)
        self.assertSensor('e_day', 42.7, 'kWh', data)
        self.assertSensor('e_total', 8556.9, 'kWh', data)
        self.assertSensor('h_total', 1313, 'h', data)
        self.assertSensor('safety_country', 2, '', data)
        self.assertSensor('safety_country_label', "DE LV with PV", '', data)
        self.assertSensor('funbit', 66, '', data)
        self.assertSensor('vbus', 744.8, 'V', data)
        self.assertSensor('vnbus', 371.6, 'V', data)
...
class GW20K_SDT_20_Test(DtMock):
    ...
    def test_GW20K_SDT_20_runtime_data(self):
        ...
        self.assertSensor(
            "timestamp",
            datetime.strptime("2026-03-26 08:48:30", "%Y-%m-%d %H:%M:%S"),
            "",
            data,
        )
        self.assertSensor("vpv1", 677.1, "V", data)
        self.assertSensor("ipv1", 2.0, "A", data)
        self.assertSensor("ppv1", 1354, "W", data)
        ...
        self.assertSensor("vline3", 402.4, "V", data)
        self.assertSensor("vgrid1", 232.4, "V", data)
        self.assertSensor("vgrid2", 233.3, "V", data)
        self.assertSensor("vgrid3", 231.4, "V", data)
        self.assertSensor("igrid1", 8.5, "A", data)
        self.assertSensor("igrid2", 8.7, "A", data)
        self.assertSensor("igrid3", 8.6, "A", data)
        self.assertSensor("fgrid1", 49.98, "Hz", data)
        self.assertSensor("fgrid2", 49.97, "Hz", data)
        self.assertSensor("fgrid3", 49.98, "Hz", data)
        self.assertSensor("pgrid1", 1975, "W", data)
        self.assertSensor("pgrid2", 2030, "W", data)
        self.assertSensor("pgrid3", 1990, "W", data)
        self.assertSensor("total_inverter_power", 4461, "W", data)
        self.assertSensor("work_mode", 1, "", data)
        self.assertSensor("work_mode_label", "Normal", "", data)
        self.assertSensor("error_codes", 0, "", data)
        self.assertSensor("warning_code", 0, "", data)
        self.assertSensor("apparent_power", 5840, "VA", data)
        self.assertSensor("reactive_power", -3793, "var", data)
        self.assertSensor("total_input_power", 0, "W", data)
        self.assertSensor("power_factor", 0.763, "", data)
        self.assertSensor("temperature", 34.3, "C", data)
        self.assertSensor("temperature_heatsink", 35.3, "C", data)
        self.assertSensor("e_day", 4.1, "kWh", data)
        self.assertSensor("e_total", 8096.4, "kWh", data)
        self.assertSensor("h_total", 1271, "h", data)
        self.assertSensor("safety_country", 2, "", data)
        self.assertSensor("safety_country_label", "DE LV with PV", "", data)
        self.assertSensor("funbit", 66, "", data)
        self.assertSensor("vbus", 727.9, "V", data)
        self.assertSensor("vnbus", 367.8, "V", data)
        self.assertSensor("derating_mode", 0, "", data)
        self.assertSensor("derating_mode_label", "", "", data)
