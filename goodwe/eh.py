...
class EH(Inverter):
    ...
    __sensors: Tuple[Sensor, ...] = (
        ...
        Sensor("house_consumption", 0, lambda data, _: read_power(data, 10) + read_power(data, 18) + round(
            read_voltage(data, 160) * read_current(data, 162)) - read_power(data, 78), "W", "House Comsumption",
               SensorKind.ac, ),
    )
